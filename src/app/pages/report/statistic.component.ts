...
import { ReportService } from '@service/index';
...
export class StatisticComponent implements OnInit {
  ...
  onSubmit(dataForm: any) {
    ...
    if (dataForm) {
      ...
      formData.append(
        'fromDate',
        moment(dataForm.fromDate, 'DD-MM-YYYY').format('YYYY-MM-DD')
      );

      formData.append(
        'toDate',
        moment(dataForm.toDate, 'DD-MM-YYYY').format('YYYY-MM-DD')
      );

      this.errMessage = '';
      this.successMessage = '';

      this.reportService.reportStatistic(formData)
        .subscribe((data: any) => {
          const file = new Blob(
            [data], {
